...
if (!extractedDir || !outputPath) {
  console.error("Usage: node scripts/extract-all-mep-sheets.mjs <xlsx-dir> <output.csv>");
  process.exit(1);
}

const EXCLUDED_VISIBLE_SHEETS = new Set(["EjecuciÃ³n MEP", "Ejecución MEP", "Input ADU"]);
const KITCHEN_ID_BY_SHEET = new Map([
...
const sheets = workbookSheets(workbookXml)
  ...
  .filter((sheet) => !EXCLUDED_VISIBLE_SHEETS.has(sheet.name))
  .filter((sheet) => !sheet.name.toLowerCase().startsWith("input"));
...
for (const sheet of sheets) {
  ...
  const kitchenId = KITCHEN_ID_BY_SHEET.get(sheet.name) || matrix[1]?.[1] || "";
